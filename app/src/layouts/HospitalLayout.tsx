...
const HospitalLayout = () => {
  ...
  const navigation = [
    ...
    { name: 'Doctors', href: '/hospital/doctors', icon: UserPlus, badge: '8' },
    { name: 'Patients', href: '/hospital/patients', icon: Users, badge: '156' },
    { name: 'Appointments', href: '/hospital/appointments', icon: Calendar },
    { name: 'Lab Results', href: '/hospital/lab-results', icon: FlaskConical, badge: '5' },
    { name: 'Analytics', href: '/hospital/analytics', icon: BarChart3 },
    { name: 'Billing', href: '/hospital/billing', icon: CreditCard },
          ...
          <div className="space-y-1">
            {navigation.map((item) => (
              ...
              >
                ...
                {item.badge && (
                  <Badge className="bg-violet-100 text-violet-700 hover:bg-violet-100">{item.badge}</Badge>
                )}
              </Link>
            ))}
          </div>
        ...
        <div className="border-t p-4">
          <DropdownMenu>
              ...
              <button className="flex items-center gap-3 w-full p-2 rounded-xl hover:bg-slate-100 transition-colors">
                <Avatar className="w-10 h-10">
                  <AvatarFallback className="bg-violet-100 text-violet-700">
                    {user?.avatar || user?.name?.charAt(0) || 'H'}
                  </AvatarFallback>
                </Avatar>
                <div className="flex-1 text-left">
                  <p className="text-sm font-medium text-slate-900">{user?.name || 'Hospital Admin'}</p>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem onClick={() => navigate('/hospital/billing')}>
                ...
                <div className="flex flex-col h-full">
                  <div className="p-4 border-b">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-slate-900">{user?.name || 'Hospital Admin'}</p>
                        <p className="text-xs text-slate-500">{user?.email}</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-1">
                      {navigation.map((item) => (
                        ...
                        >
                          ...
                          <span className="flex-1">{item.name}</span>
                          {item.badge && (
                            <Badge className="bg-violet-100 text-violet-700">{item.badge}</Badge>
                          )}
                        </Link>
                      ))}
                    </div>
