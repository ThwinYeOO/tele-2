...
const PatientLayout = () => {
  ...
  const navigation = [
    ...
    { name: 'My Consultations', href: '/patient/consultations', icon: Video },
    { name: 'Medical Records', href: '/patient/records', icon: FileText },
    { name: 'Prescriptions', href: '/patient/prescriptions', icon: Pill },
    { name: 'My Wallet', href: '/patient/wallet', icon: Wallet },
    { name: 'Emergency SOS', href: '/patient/emergency', icon: AlertCircle, danger: true },
          ...
          <div className="space-y-1">
            {navigation.map((item) => (
              ...
              >
                ...
                {item.badge && (
                  <Badge className="bg-teal-100 text-teal-700 hover:bg-teal-100">{item.badge}</Badge>
                )}
              </Link>
            ))}
          </div>
        ...
        <div className="border-t p-4">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-3 w-full p-2 rounded-xl hover:bg-slate-100 transition-colors">
                ...
                <div className="flex-1 text-left">
                  <p className="text-sm font-medium text-slate-900">{user?.name || 'Patient'}</p>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem onClick={() => navigate('/patient/wallet')}>
                ...
                <div className="flex flex-col h-full">
                  <div className="p-4 border-b">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-slate-900">{user?.name || 'Patient'}</p>
                        <p className="text-xs text-slate-500">{user?.email}</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-1">
                      {navigation.map((item) => (
                        ...
                        >
                          ...
                          <span className="flex-1">{item.name}</span>
                          {item.badge && (
                            <Badge className="bg-teal-100 text-teal-700">{item.badge}</Badge>
                          )}
                        </Link>
                      ))}
                    </div>
